...
void MainWindow::slot_read_word(const QString &sLine)
{
    ...
    ui->labelWord->setText(sLine.split(" ").at(0));
...
void MainWindow::on_btnCtrl_clicked()
{
    ...
    default: {
        ...
        m_pReadThread->config(ui->boxGrade->currentText(),
                              std::make_shared<QStringList>(m_slLineRead));
        m_pReadThread->start();
        ui->btnCtrl->setText("暂停");
...
void MainWindow::on_listView_clicked(const QModelIndex &index)
{
    m_pReadThread->pause();
    ui->btnCtrl->setText("继续");
    m_pSingleReadThread->set(std::make_shared<QString>(ui->boxGrade->currentText()),
                             std::make_shared<QString>(m_slWordShow.at(index.row())));
...
{
    QString srcName = QCoreApplication::applicationDirPath() + "/" +
            ui->boxGrade->currentText() + ".words/allwords.txt";
    QString destName = QCoreApplication::applicationDirPath() + "/" +
            ui->boxGrade->currentText() + ".words/" +
            ui->boxLetter->currentText() + ".txt";
